...
with DAG(
    dag_id="roughness",
    description="Compute roughness from remote COG using GDAL",
    ...
    params={
        ...
        "compute_edges": Param(False, type="boolean", description="Compute roughness at edges"),
    },
    tags=["raster", "roughness", "gdal", "cog"],
) as dag:

    @task()
    def run_roughness(input_s3_url: str, band: int, compute_edges: bool) -> str:
        ...
        tmp_roughness = f"/tmp/roughness_{uuid.uuid4().hex}.tif"
        tmp_out = f"/tmp/roughness_cog_{uuid.uuid4().hex}.tif"
        
        print(f"Downloading {input_s3_url} to {tmp_input}")
        s3.download_file(bucket, key, tmp_input)
        
        # Compute roughness using GDAL
        print("Computing roughness with GDAL...")
        cmd = ["gdaldem", "roughness", tmp_input, tmp_roughness, "-of", "GTiff"]
        ...
        print("GDAL roughness completed successfully")
        
        # Convert to COG
        print("Converting roughness to COG...")
        cog_cmd = [
            "gdal_translate", tmp_roughness, tmp_out,
        ...
        os.remove(tmp_input)
        os.remove(tmp_roughness)

        # Upload back to S3
        out_key = f"{user_id}/outputs/roughness/{uuid.uuid4().hex}.tif"
