...
with DAG(
    dag_id="slope",
    description="Compute slope from remote COG using GDAL",
    ...
    params={
        ...
        "compute_edges": Param(False, type="boolean", description="Compute slope at edges"),
    },
    tags=["raster", "slope", "gdal", "cog"],
) as dag:

    @task()
    def run_slope(input_s3_url: str, band: int, scale: float, compute_edges: bool) -> str:
        ...
        tmp_slope = f"/tmp/slope_{uuid.uuid4().hex}.tif"
        tmp_out = f"/tmp/slope_cog_{uuid.uuid4().hex}.tif"
        
        print(f"Downloading {input_s3_url} to {tmp_input}")
        s3.download_file(bucket, key, tmp_input)
        
        # Compute slope using GDAL
        print("Computing slope with GDAL...")
        cmd = [
            "gdaldem", "slope", tmp_input, tmp_slope,
        ...
        print("GDAL slope completed successfully")
        
        # Convert to COG
        print("Converting slope to COG...")
        cog_cmd = [
            "gdal_translate", tmp_slope, tmp_out,
        ...
        # Upload back to S3
        out_key = f"{user_id}/outputs/slope/{uuid.uuid4().hex}.tif"
